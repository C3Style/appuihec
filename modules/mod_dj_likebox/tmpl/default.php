...
if(!$params->get('add_css')) {
	...
	if(!$params->get('footer',0)) {
		echo '<div style="overflow:hidden; width: '.$params->get('width').'px; ';
		echo 'height: '.($params->get('height') - 25).'px;">';
		$params->set('width',$params->get('width') + 12);
	}
	
	echo '<iframe src="http://www.facebook.com/plugins/likebox.php?';
	...
	if(!$params->get('footer',0)) echo 'margin: -1px -6px 0; ';
	echo 'height: '.$params->get('height').'px;" allowTransparency="true"></iframe>';
	
	if(!$params->get('footer',0)) echo '</div>';

} else {
?>

<div id="fb-root"></div>
<script src="http://connect.facebook.net/<?php echo $language_tag; ?>/all.js#xfbml=1"></script>
<fb:fan
