 ...
 * @version $Id: documents.php 1367 2010-05-04 13:38:22Z mathias $
 * @package DOCman_1.4
...
function saveDocument($uid)
{
    ...
    $doc = new mosDMDocument($database);
    $doc->load((int) mosGetParam($_REQUEST, 'id', 0)); // Load from id

    // get the uploaded filename from the session
	if(isset($_SESSION['docman.dmfilename']))
	{
		$_POST['dmfilename']	= @$_SESSION['docman.dmfilename'];
		$_POST['document_url']	= @$_SESSION['docman.document_url'];
	} else
	{
    ...
    if ($doc->save())
    {
    	unset($_SESSION['docman.dmfilename']);
		unset($_SESSION['docman.document_url']);
